...
if(option != None):
    ...
    with tab2:
        ...
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.1,
                            subplot_titles=("RSI", "Close Price"))

        # Add RSI line chart in the first row
        fig.add_trace(go.Scatter(x=rsi.index, y=rsi, mode='lines', name='RSI'), row=1, col=1)
        fig.add_hline(y=30, line_dash="dash", line_color="green", row=1, col=1)
        fig.add_hline(y=70, line_dash="dash", line_color="red", row=1, col=1)

        # Add stock price line chart in the second row
        fig.add_trace(go.Scatter(x=closep_stock.index, y=closep_stock["Close"], mode='lines', name='Close Price'), row=2, col=1)
    ...
    with tab3:
        st.markdown("**Relevant Stocks**")
        with st.spinner(text="Loading the correlation stocks..."):
